...
*/
/**
常量的定义： 和php一样，使用const 关键字定义常量；常见的定义如下
 */
const Pi float64 = 3.141592644444444444444444444444444444444
const zero = 0.0  //  无类型浮点常量
const (           //通过一个const 定义多个常量，和 var类似
	size int64 = 1024
	eof = -1  // 无类型整型常量
)
...
//  注意：  由于常量的赋值是一个编译期行为，所以右值不能出现任何需要运行期才能得出的结果的表达式；
 ...
 */
/*
Go语言预定义了true，false， iota这些常量
    iota比较特殊，可以被认为是一个可被编译器修改的常量，在每一个const关键字出现时被重置为0；
           然后再下一个const出现之前，没出现一次iota，其所代表的数字会自动增1；
*/
//---------------------------------
const (
	c0 = iota      //  0
...
const (
	w0 = iota * 2;    // 0
	w1 = iota * 2;    // 2   只要可上面一行的表达式一样，是可以省略后一个赋值表达式
	w2 = iota * 2;    // 4
)
...
*/
//   常量还可以用于枚举；
//   枚举中包含一系列相关的常量，例如：关于一个星期中每天的定义，Go语言并不支持其他语言用于表示枚举的enum关键字
//   而是通过 const 后跟一对圆括号定义一组常量的方式来实现枚举

//   PHP本身不支持枚举，但可以通过SPL库提供的 SplEnum 类实现，还有第三方扩展包 myclabs/php-enum 可用来实现对枚举的支持
...
//   和函数体外声明的变量一样，以大写字母开头的常量再包外可见（类似public修饰的类属性）；
//    例如 上面的Pi，Sunday等等
//   而以小写字母开头的常量只能再包内访问（类似 protected 修饰的类属性） 例如：zero、numberOfDays 等
 //   函数体内声明的常量只能再函数体内生效
